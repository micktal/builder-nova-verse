...
import { 
  Heart, ArrowLeft, Play, Pause, CheckCircle, Home, 
  Zap, RotateCcw, Volume2, Timer, Waves
} from 'lucide-react';
...
const Sequence2 = () => {
  ...
  const techniques = [
    {
      ...
      details: 'Technique simple et efficace pour calmer le système nerveux autonome. L\'expiration plus longue active le système parasympathique.',
      instructions: ['Asseyez-vous confortablement', 'Inspirez par le nez pendant 4 secondes', 'Expirez par la bouche pendant 6 secondes', 'Répétez 10 cycles'],
      benefits: 'Réduction immédiate du stress, amélioration de la concentration',
      color: 'calm',
      duration: 60
    },
    {
      ...
      details: 'Technique avancée développée par le Dr. Andrew Weil. Très efficace pour l\'endormissement et la gestion de l\'anxiété.',
      instructions: ['Placez la langue derrière les dents', 'Inspirez par le nez pendant 4 secondes', 'Retenez votre souffle pendant 7 secondes', 'Expirez complètement pendant 8 secondes'],
    ...
    },
    {
      ...
      details: 'Technique de Jacobson qui permet de distinguer tension et relaxation musculaire pour un relâchement profond.',
      instructions: ['Commencez par les pieds', 'Contractez le muscle 5 secondes', 'Relâchez brutalement', 'Observez la différence de sensation'],
    ...
    },
    {
      ...
      details: 'Courtes pauses préventives pour éviter l\'accumulation de stress et maintenir l\'énergie tout au long de la journée.',
      instructions: ['Levez-vous et étirez-vous', 'Respirez profondément 3 fois', 'Bougez les épaules en cercle', 'Hydratez-vous'],
  ...
  const dragDropPairs = [
    ...
    { technique: 'Relaxation progressive', effect: 'Relâche les tensions musculaires' },
  ...
  const startBreathing = (techniqueId: string) => {
    ...
    const phases = techniqueId === 'breathing-4-7-8' 
      ? [{ phase: 'inhale', duration: 4000 }, { phase: 'hold', duration: 7000 }, { phase: 'exhale', duration: 8000 }]
      : [{ phase: 'inhale', duration: 4000 }, { phase: 'exhale', duration: 6000 }];
  ...
  const isSequenceComplete = completedTechniques.length >= 2 && dragDropComplete;
    ...
    <div className="max-w-4xl mx-auto text-center mb-8">
      <div className="bg-white rounded-2xl p-8 shadow-lg border border-nature-200">
        ...
        <p className="text-xl text-gray-600 mb-6 leading-relaxed">
          Apprenez à maîtriser votre respiration et à relâcher vos tensions corporelles 
          grâce à des techniques scientifiquement prouvées.
        </p>
        <div className="grid md:grid-cols-4 gap-4 mb-6">
          {techniques.map((technique, index) => (
            <div key={technique.id} className="text-center p-4 bg-gray-50 rounded-lg">
              <div className={`w-8 h-8 mx-auto mb-2 bg-${technique.color}-100 text-${technique.color}-600 rounded-full flex items-center justify-center`}>
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mb-4">
          Cliquez sur chaque technique pour découvrir ses instructions détaillées et ses bénéfices.
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-6 mb-8">
        {techniques.map((technique, index) => (
          <Card key={technique.id} className={`border-2 border-${technique.color}-200 hover:border-${technique.color}-300 transition-all`}>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className={`p-3 rounded-lg bg-${technique.color}-100 text-${technique.color}-600`}>
                  <Heart className="w-6 h-6" />
                </div>
                <Badge className={`bg-${technique.color}-100 text-${technique.color}-700`}>
                  {technique.duration}s
                </Badge>
              </div>
  ...
  const PracticeStep = () => (
    <div className="max-w-6xl mx-auto">
      <div className="mb-6">
        ...
        <Progress value={(completedTechniques.length / 2) * 50 + (dragDropComplete ? 50 : 0)} className="h-3 mb-4" />
      </div>

      {/* Breathing Animation Center */}
      <Card className="mb-8">
        <CardContent className="p-8 text-center">
          <h3 className="text-xl font-semibold mb-6">Animation de respiration guidée</h3>
          <div className="flex justify-center mb-6">
            <div className="relative w-48 h-48">
              <div 
                className={`w-full h-full rounded-full transition-all duration-1000 ${getBreathingAnimation()} ${
                  isBreathing 
                    ? (breathingPhase === 'inhale' ? 'bg-gradient-to-br from-calm-400 to-calm-600' : 
                       breathingPhase === 'hold' ? 'bg-gradient-to-br from-yellow-400 to-orange-500' :
                       'bg-gradient-to-br from-nature-400 to-nature-600')
                    : 'bg-gradient-to-br from-gray-300 to-gray-400'
                }`}
                style={{
                  boxShadow: isBreathing ? '0 0 40px rgba(59, 130, 246, 0.6)' : '0 10px 25px rgba(0,0,0,0.1)'
                }}
              />
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        {techniques.slice(0, 2).map((technique) => (
          <Card key={technique.id} className={`border-2 transition-all ${
                    ...
                    {Object.values(matches).includes(pair.effect) ? (
                      <div className="text-green-700 font-medium">
                        <span className="text-sm">
                          {Object.keys(matches).find(key => matches[key] === pair.effect)}
                        </span>
                        <br />
          ...
          {dragDropComplete && (
            <div className="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              <p className="text-green-600 text-sm mt-1">
                Vous maîtrisez maintenant l'association entre techniques et effets thérapeutiques.
              </p>
            </div>
          )}
        </CardContent>
        ...
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700 mb-4">
              Vous avez maîtrisé les techniques physiologiques de base. 
              Prêt pour les techniques cognitives ?
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="bg-gradient-to-br from-green-50 to-nature-50 border-green-200">
        <CardContent className="p-8">
          ...
          <p className="text-lg text-gray-700 mb-6 leading-relaxed">
            Vous maîtrisez maintenant les techniques physiologiques de gestion du stress. 
            Ces outils vous permettront de réguler votre système nerveux en toutes circonstances.
          </p>
          
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white p-6 rounded-lg shadow-sm">
              <h4 className="font-bold text-gray-900 mb-3">Techniques acquises :</h4>
