...
import { 
  Brain, Heart, Lightbulb, Users, FileText, Play, Pause, 
  MousePointer, DragHandleDots2Icon, CheckCircle, AlertCircle,
  Download, RotateCcw, Volume2
} from 'lucide-react';

// Sequence 1: Understanding Triggers
export const TriggersSequence = ({ onComplete }: { onComplete: () => void }) => {
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Explorez cette scène de bureau pour découvrir les 4 catégories de déclencheurs
        </p>
      </div>
      ...
      <Card className="mb-6">
        <CardContent className="p-6">
          ...
          <div className="mt-4">
            ...
            <Progress value={(viewedHotspots.length / 4) * 100} className="h-2" />
          </div>
        </CardContent>
      </Card>
...
export const PhysiologicalSequence = ({ onComplete }: { onComplete: () => void }) => {
                  ...
                  >
                    {Object.values(matches).includes(effect) ? (
                      <span className="text-green-700 font-medium">
                        {Object.keys(matches).find(key => matches[key] === effect)} → {effect}
                      </span>
                    ) : (
          ...
          <div className="flex justify-center mb-6">
            <div className="relative w-40 h-40">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-white font-semibold">
                {isBreathing ? (breathingPhase === 'inhale' ? 'Inspirez' : 'Expirez') : 'Prêt'}
              </div>
            </div>
          </div>
          {activeExercise && (
            <p className="text-sm text-gray-600 mb-4">
              Exercice en cours: {techniques.find(t => t.id === activeExercise)?.name}
            </p>
          )}
        </CardContent>
      </Card>
