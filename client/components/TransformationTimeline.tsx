...
const TransformationTimeline = () => {
  const [activeWeek, setActiveWeek] = useState<'week1' | 'week2' | 'both'>('both');
      ...
      days: [
        {
          ...
          details: 'Boîte mail débordante dès le matin. Difficile de prioriser, sentiment d\'urgence constant.',
          emoji: '😰'
        },
        {
          day: 'Mardi', 
          ...
          details: 'Planning bouleversé, pas de pause entre les réunions. Fatigue mentale qui s\'accumule.',
          emoji: '😵'
        },
        {
          day: 'Mercredi',
          ...
          details: 'Malentendu avec un collègue. Ruminations pendant la journée, concentration perturbée.',
          emoji: '😤'
        },
        {
          day: 'Jeudi',
          ...
          details: 'Nuit agitée à cause du stress. Erreurs répétées, irritabilité croissante.',
          emoji: '🥱'
        },
        {
          day: 'Vendredi',
          ...
          details: 'Course contre la montre pour finir un projet. Tension maximale, snacking compulsif.',
          emoji: '😡'
        },
        {
          day: 'Samedi',
          ...
          details: 'Impossible de décrocher du travail. Pensées en boucle sur les problèmes de la semaine.',
          emoji: '🤯'
        },
        {
          day: 'Dimanche',
          ...
          details: 'Angoisse du dimanche soir. Sommeil difficile, appréhension du lundi.',
          emoji: '😟'
        }
      ]
    },
      ...
      days: [
        {
          ...
          details: 'Routine matinale avec respiration. Emails triés méthodiquement, priorités claires.',
          emoji: '😌'
        },
        {
          day: 'Mardi',
          ...
          details: 'Micro-pauses entre réunions. Étirements de 2 min, énergie maintenue toute la journée.',
          emoji: '😊'
        },
        {
          day: 'Mercredi',
          ...
          details: 'Tâches classées par urgence/importance. Sensation de contrôle, efficacité accrue.',
          emoji: '😎'
        },
        {
          day: 'Jeudi',
          ...
          details: 'Problème reformulé positivement. Focus sur les solutions plutôt que les obstacles.',
          emoji: '🤔'
        },
        {
          day: 'Vendredi',
          ...
          details: 'Modèle DESC appliqué avec succès. Discussion franche et respectueuse avec un collègue.',
          emoji: '😇'
        },
        {
          day: 'Samedi',
          ...
          details: 'Session sport de 30 min. Endorphines naturelles, évacuation des tensions.',
          emoji: '🏃‍♂️'
        },
        {
          day: 'Dimanche',
          ...
          details: 'Relaxation progressive avant le coucher. Nuit paisible, réveil en forme.',
          emoji: '😴'
        }
      ]
    }
  ...
  const TimelineSVG = ({ week, weekKey }: { week: typeof weekData.week1, weekKey: 'week1' | 'week2' }) => {
    const width = 600;
    const height = 120;
    const padding = 40;
    const pointRadius = 6;
    ...
    return (
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
        <defs>
          <linearGradient id={`gradient-${weekKey}`} x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor={weekKey === 'week1' ? '#ef4444' : '#10b981'} stopOpacity="0.3" />
            <stop offset="100%" stopColor={weekKey === 'week1' ? '#ef4444' : '#10b981'} stopOpacity="0.1" />
          </linearGradient>
        </defs>
        ...
        {[0, 25, 50, 75, 100].map(value => (
          <text
            key={value}
            x={padding - 10}
            y={height - padding - ((value / maxStress) * (height - 2 * padding)) + 4}
            textAnchor="end"
            fontSize="10"
            fill="#9ca3af"
          >
  ...
  const DayDetail = ({ weekKey, dayIndex }: { weekKey: 'week1' | 'week2', dayIndex: number }) => {
    const day = weekData[weekKey].days[dayIndex];
    const Icon = day.icon;

    return (
      <Card className={`border-2 ${weekKey === 'week1' ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'} animate-slide-up`}>
        <CardContent className="p-6">
          <div className="flex items-center gap-4 mb-4">
            <div className={`p-3 rounded-full ${weekKey === 'week1' ? 'bg-red-100' : 'bg-green-100'}`}>
              <Icon className={`w-6 h-6 ${weekKey === 'week1' ? 'text-red-600' : 'text-green-600'}`} />
            </div>
            <div>
  ...
  return (
    <div ref={timelineRef} className="max-w-6xl mx-auto px-6 py-16 bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl">
      <div className="text-center mb-12">
        ...
        <p className="text-xl text-gray-600 max-w-4xl mx-auto leading-relaxed">
          Découvrez la transformation concrète d'une semaine type grâce à l'application 
          des techniques de régulation du stress. Une évolution mesurable jour après jour.
        </p>
      </div>
          ...
          <Card className="border-red-200 bg-red-50/50">
            <CardHeader className="pb-4">
              <CardTitle className="flex items-center gap-2 text-red-800">
                ...
                <Badge variant="destructive" className="ml-auto">Stress croissant</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-green-50 border-blue-200 mb-8">
        <CardContent className="p-8">
          <h3 className="text-2xl font-bold text-center mb-6">Résultats de la transformation</h3>
