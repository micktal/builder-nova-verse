...
import { 
  Lightbulb, Users, FileText, CheckCircle, AlertCircle, Download,
  ArrowRight, MessageSquare, ThumbsUp, ThumbsDown, Save
} from 'lucide-react';

// Sequence 3: Cognitive and Organizational Techniques
export const CognitiveSequence = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const [reframingExamples, setReframingExamples] = useState<{[key: string]: boolean}>({});
          ...
          {reframingScenarios.map((scenario) => (
            <div key={scenario.id} className="border rounded-lg p-4">
              ...
              <div className="mt-4 flex justify-center gap-4">
                <Button
                  variant={reframingExamples[scenario.id] === true ? "default" : "outline"}
                  size="sm"
                ...
                <Button
                  variant={reframingExamples[scenario.id] === false ? "default" : "outline"}
                  size="sm"
              ...
              {reframingExamples[scenario.id] === true && (
                <div className="mt-3 p-3 bg-blue-50 rounded border border-blue-200">
                  <p className="text-blue-700 text-sm">
                    ✓ Excellent ! Cette reformulation aide à réduire le stress et encourage l'action constructive.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
            <p className="text-yellow-800 text-sm">
              💡 <strong>Astuce :</strong> Commencez toujours par la tâche la plus difficile ou importante 
              quand votre énergie est au maximum.
            </p>
          </div>
        </div>
      </CardContent>
...
export const AssertiveSequence = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const scenarios = [
    {
      id: 1,
      context: "Votre manager vous demande de rester tard pour finir un projet non urgent",
      options: [
        {
          id: 'aggressive',
          text: "Non, j'ai déjà assez travaillé aujourd'hui !",
          type: 'Agressif',
          feedback: "Cette réponse peut créer des tensions. L'assertivité cherche un équilibre."
        },
        {
          id: 'passive',
          text: "D'accord... je vais rester...",
          type: 'Passif', 
          feedback: "Vous acceptez au détriment de votre bien-être. Il faut apprendre à dire non."
        },
        {
          id: 'assertive',
          text: "Je comprends l'importance du projet. Puis-je proposer de commencer tôt demain matin ?",
          type: 'Assertif',
          feedback: "Parfait ! Vous reconnaissez la demande tout en proposant une alternative."
        }
      ]
    },
    {
      id: 2,
      context: "Un collègue vous interrompt constamment pendant vos explications en réunion",
      options: [
        {
          id: 'passive',
          text: "Je continue à parler en espérant qu'il s'arrête",
          type: 'Passif',
          feedback: "Ignorer le problème ne le résout pas. Il faut exprimer vos besoins."
        },
        {
          id: 'assertive',
          text: "J'aimerais finir mon point avant d'entendre ton retour",
          type: 'Assertif',
          feedback: "Excellent ! Vous exprimez clairement votre besoin de manière respectueuse."
        },
        {
          id: 'aggressive',
          text: "Tu peux me laisser parler s'il te plaît !",
          type: 'Agressif',
          feedback: "Le ton est trop directif. L'assertivité reste respectueuse même dans l'affirmation."
        }
      ]
    }
  ];
      ...
      <CardContent>
        <div className="grid md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-calm-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">Les faits objectifs sans jugement</p>
          </div>
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <h5 className="font-semibold text-blue-800 mb-2">Exemple concret :</h5>
  ...
  const ScenarioPlayer = () => (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>
          Scénario {currentScenario + 1}/{scenarios.length} - Choisissez votre réponse
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
...
export const ActionPlanSequence = ({ onComplete }: { onComplete: () => void }) => {
...
Date: ${new Date().toLocaleDateString('fr-FR')}
  ...
  const QuizFinal = () => {
    ...
    const questions = [
      ...
      {
        question: "Dans la matrice d'Eisenhower, que faire des tâches urgentes et importantes ?",
        options: [
          "Les déléguer",
          "Les planifier", 
      ...
      {
        question: "Quelle technique de respiration aide à calmer le système nerveux ?",
        options: [
          "Respiration rapide",
          "Retenir son souffle",
          "Respiration 4-6",
    ...
    const score = Object.entries(answers).reduce((total, [qIndex, answer]) => {
      return total + (parseInt(answer) === questions[parseInt(qIndex)].correct ? 1 : 0);
    }, 0);
          ...
          <CardContent>
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                {score >= 4 ? "Excellent ! Vous maîtrisez les concepts." :
                 score >= 3 ? "Bien ! Quelques révisions et ce sera parfait." :
                 "Bon début ! Relisez les modules pour consolider."}
              </p>
              <Badge className={score >= 4 ? "bg-green-500" : score >= 3 ? "bg-blue-500" : "bg-yellow-500"}>
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              <label className="block font-semibold mb-2">Mes principaux déclencheurs :</label>
              <Textarea
            ...
            <div>
              <label className="block font-semibold mb-2">Techniques préférées :</label>
              <Textarea
                value={actionPlan.techniques}
                onChange={(e) => setActionPlan(prev => ({ ...prev, techniques: e.target.value }))}
                placeholder="Ex: Respiration 4-6, matrice d'Eisenhower..."
                className="min-h-[80px]"
              />
            </div>
            <div>
              <label className="block font-semibold mb-2">Moments d'application :</label>
              <Input
                value={actionPlan.moments}
                onChange={(e) => setActionPlan(prev => ({ ...prev, moments: e.target.value }))}
                placeholder="Ex: Début de journée, avant réunions importantes..."
              />
            </div>
            <div>
              <label className="block font-semibold mb-2">Résultats attendus :</label>
              <Input
                value={actionPlan.results}
                onChange={(e) => setActionPlan(prev => ({ ...prev, results: e.target.value }))}
        ...
        <CardContent>
              ...
              <button
                ...
                onClick={() => {
                  if (routineElements.includes(option)) {
                    setRoutineElements(prev => prev.filter(el => el !== option));
          ...
          {routineElements.length > 0 && (
            <div className="bg-blue-50 p-4 rounded-lg">
              <h5 className="font-semibold text-blue-800 mb-2">Votre routine sélectionnée :</h5>
      ...
      {/* Export Button */}
      {(actionPlan.triggers || actionPlan.techniques || routineElements.length > 0) && (
